...
from modules.Z.MOD_ZZ_Z import MOD_ZZ_Z
from modules.Z.ADD_ZZ_Z import ADD_ZZ_Z  
...
class TestMUL_ZZ_Z:
    ...
    def test_positive_multiplication(self):
        ...
        result = MUL_ZZ_Z(num1, num2)
        assert result.b == 0  # положительный результат
        assert result.A == [5]  # 15 -> [5, 1] в правильном порядке?
...
class TestArithmeticRelationships:
    ...
    def test_relationship_between_div_and_mod_positive(self):
        ...
        div_result = DIV_ZZ_Z(a, b)  # 17 // 5 = 3
        mod_result = MOD_ZZ_Z(a, b)  # 17 % 5 = 2

        # Вычисляем b * div_result
        b_times_div = MUL_ZZ_Z(b, div_result)  # 5 * 3 = 15

        # Вычисляем b * div_result + mod_result
        sum_result = ADD_ZZ_Z(b_times_div, mod_result)  # 15 + 2 = 17
    ...
    def test_relationship_between_div_and_mod_negative_dividend(self):
        ...
        div_result = DIV_ZZ_Z(a, b)  # -17 // 5
        mod_result = MOD_ZZ_Z(a, b)  # -17 % 5

        # Проверяем соотношение: a = b * div_result + mod_result
        b_times_div = MUL_ZZ_Z(b, div_result)
        sum_result = ADD_ZZ_Z(b_times_div, mod_result)
    ...
    def test_relationship_between_div_and_mod_zero_dividend(self):
        ...
        div_result = DIV_ZZ_Z(a, b)  # 0 // 5 = 0
        mod_result = MOD_ZZ_Z(a, b)  # 0 % 5 = 0

        # Проверяем соотношение: 0 = 5 * 0 + 0
        b_times_div = MUL_ZZ_Z(b, div_result)  # 5 * 0 = 0
        sum_result = ADD_ZZ_Z(b_times_div, mod_result)  # 0 + 0 = 0
...
def test_comprehensive_arithmetic_chain():
    ...
    mul_result = MUL_ZZ_Z(x, y)  # 8 * 3 = 24
    div_result = DIV_ZZ_Z(mul_result, y)  # 24 // 3 = 8
    mod_result = MOD_ZZ_Z(mul_result, y)  # 24 % 3 = 0
    ...
    check_result = ADD_ZZ_Z(MUL_ZZ_Z(y, div_result), mod_result)
