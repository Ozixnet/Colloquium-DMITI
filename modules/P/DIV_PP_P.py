...
def DIV_PP_P_f(arg1: PNum, arg2:PNum) -> PNum:
    ...
    while deg1 >= deg2:
        ...
        if deg1 == deg2:
            res = ADD_PP_P_f(res, PNum(0, list(DIV_QQ_Q_f(arg1.C[-1], arg2.C[-1]))))
        else:
        #иначе вычитаем из делимого делитель умноженный x^(разница их степеней)
            res = ADD_PP_P_f(res, SUB_PP_P_f(arg1, MUL_Pxk_P_f(arg2, deg1 - deg2)))
            arg1 = SUB_PP_P_f(arg1, MUL_Pxk_P_f(arg2, deg1 - deg2))
        deg1 = DEG_P_N_f(arg1)
        deg2 = DEG_P_N_f(arg2)
    return res